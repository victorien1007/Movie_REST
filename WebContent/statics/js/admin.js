...
$(function() {
	...
	$(document).on(click, '#guide', function() {
		...
		$('#content').toggleClass('toggle-left');
	});
	// 侧边栏二级菜单
...
var Tab = {
	addTab: function(title, url) {
		...
		if ($('#tab_' + index).length == 0) {
			...
			// 添加iframe
			$('.iframe').removeClass('cur');
			var setheight = document.documentElement.clientHeight - 118;
			var iframe = '<div id="iframe_' + index + '" class="iframe cur"><iframe class="tab_iframe" src="' + url + '" width="100%" frameborder="0" scrolling="auto" onload="changeFrameHeight(this)" style="height:'+setheight+'px"></iframe></div>';
